...
The `install.sh` script verifies dependencies (`python3`, `ffmpeg`), installs packages from `requirements.txt`, and creates a global symlink in `~/.local/bin/rightsub`.
...
The formula is pre-built in `Formula/rightsub.rb`.

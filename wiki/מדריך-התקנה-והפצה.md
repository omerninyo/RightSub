...
סקריפט ההתקנה `install.sh` בודק את קיום התלויות (`python3`, `ffmpeg`), מתקין את הספריות הנדרשות מ-`requirements.txt`, ומייצר קישור הרצה סימבולי גלובלי ב-`~/.local/bin/rightsub`.
...
לשם כך נוצר הקובץ `Formula/rightsub.rb`.

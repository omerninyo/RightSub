...
def extract_from_video(video_path, output_srt=None, track_index=None, lang="eng", force=False):
    """Extract embedded subtitle track from a video container."""
...
def process_target(target_path, output=None, track_index=None, lang="eng", force=False, recursive=False):
    ...
    if target.is_file():
        ok, reason = extract_from_video(target, output_srt=output, track_index=track_index, lang=lang, force=force)
        ...
        stats = {"extracted": 0, "exists": 0, "external_found": 0, "external_converted": 0, "failed": 0}

        for i, vid in enumerate(video_files, start=1):
            print(f"[{i}/{len(video_files)}] Processing {vid.name}...")
            ok, reason = extract_from_video(vid, track_index=track_index, lang=lang, force=force)
            if ok:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Universal Subtitle Discovery & Extraction Engine (FFmpeg)."
    )

...
תיקון סימני פיסוק, המרת ג'יבריש מ-CP1255 ל-UTF-8 וניקוי פרסומות ישירות בספרייה:
```bash
# תצוגה מקדימה ללא שינוי קבצים (Dry Run):
./rightsub fix-plex /path/to/TV_Shows/ --recursive --clean-ads --dry-run

# הפעלת התיקון ישירות על הקבצים עם גיבוי אוטומטי (.srt.bak):
...
## 🏆 הוכח בקנה מידה רחב: בנצ'מרק "בוסטון ליגל"
RightSub נבחנה ואומתה על גבי כל 5 העונות של סדרת הדרמה המשפטית *בוסטון ליגל*:

...
## 🦙 הרצת תרגום מקומי מלא עם Ollama (ללא מפתחות API)

אם אינכם רוצים להשתמש במודלי ענן מסחריים (כגון Gemini או OpenAI), RightSub כוללת כעת תמיכה מובנית ב-Ollama:
...
2. הריצו תרגום ישיר של תיקיית מנות שהוכנה:
   ...
   ```
   *הסקריפט יתחבר ל-Ollama המקומי, ישלח את מנות ה-JSON, יאמת את המפתחות, וימזג ישירות לקובץ עברית מושלם לפלקס.*
